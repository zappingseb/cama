...
function getVirtulData(year) {
    year = year || '2017';
    var date = getParsedDate(year + '-01-01');
    var end = getParsedDate((year + 1) + '-01-01');
    ...
    for (var time = date; time < end; time += dayTime) {
        ...
        data.push([
            moment().format('yyyy-MM-dd',time),items.join("|")
...
function renderItem(params, api) {
    ...
    var group = {
        type: 'group',
        children: map(layouts[events.length - 1], function (itemLayout, index) {
            return {
                type: 'path',
                shape: {
                ...
                position: [
                    cellPoint[0] + linearMap(itemLayout[0], [-0.5, 0.5], [-cellWidth / 2, cellWidth / 2]),
                    cellPoint[1] + linearMap(itemLayout[1], [-0.5, 0.5], [-cellHeight / 2 + 20, cellHeight / 2])
                ],
                style: api.style({
    ...
    group.children.push({
        ...
        style: {
            ...
            text: moment.format/('dd', api.value(0)),
